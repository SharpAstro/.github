...
- **[WebGl.Renderer](https://github.com/SharpAstro/WebGl.Renderer)** — WebGL2 backend for the same `DIR.Lib` `Renderer<TSurface>`: Blazor WebAssembly, command-buffer JS interop, MSDF text. A widget written once therefore draws on the desktop through Vulkan and in a browser tab through WebGL, which is what puts the planner and sky atlas at [sharpastro.github.io/tianwen](https://sharpastro.github.io/tianwen/).
...
- **[LAN.Lib](https://github.com/SharpAstro/LAN.Lib)** — Zero-dependency LAN peer discovery: a symmetric UDP announce beacon plus a self-expiring peer table over a shared broadcast domain, filtered by service name. `TimeProvider`-driven, so beacon cadence and expiry stay deterministic in tests.
